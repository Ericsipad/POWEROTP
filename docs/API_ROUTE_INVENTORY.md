...
| `app/v1/botblocker/browser-assessment/[webhookId]/route.ts` | `/v1/botblocker/browser-assessment/{webhookId}` | `POST` | Self-validating project/site endpoint + scoped visitor token + envelope | server-to-server | Browser BotBlocker adapters through their server bridge | Planned `https://verify.powerotp.com`; master/fallback `https://api.powerotp.com` |
...
| `app/v1/botblocker/rapid-auth/[webhookId]/route.ts` | `/v1/botblocker/rapid-auth/{webhookId}` | `POST` | Self-validating project/site endpoint + initial site credential + envelope | server-to-server | BotBlocker rapid-check adapters | Planned `https://verify.powerotp.com`; master/fallback `https://api.powerotp.com` |
| `app/v1/botblocker/risk-events/[webhookId]/route.ts` | `/v1/botblocker/risk-events/{webhookId}` | `POST` | Self-validating project/site endpoint + scoped visitor token + envelope | server-to-server | BotBlocker adapters | Planned `https://verify.powerotp.com`; master/fallback `https://api.powerotp.com` |
