...
| CC3 | Risk assessment | Partially implemented | `THREAT_MODEL.md#botblocker-threat-model` is the current risk assessment; Phases 13A–13D correct the plugin-instruction/customer-enforcement boundary and document explicit-only OTP DOM effects across supported wrappers, but no formal recurring risk-assessment process exists yet |
