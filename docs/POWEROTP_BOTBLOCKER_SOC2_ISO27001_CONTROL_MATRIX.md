...
| PI1 | Processing integrity — accurate, complete, authorized processing | Partially implemented | Phases 6–17 add atomic sequence controls, request idempotency/replay rejection, scoped credential rotation, immutable policy/report history, and deterministic scoring updates. The accounting prerequisite keeps one append-only financial ledger and transactionally updates its balance projection, uses provider-qualified payment IDs and exact OTP method types, atomically writes source/referral rows, applies durable idempotency claims, enforces 31-day threshold cooldown state, and allocates each entered ad-system/day pool exactly across projects with deterministic integer-micro-USD remainder handling |
...
| A.8.2 | Privileged access rights | Partially implemented | Phase 8 BotBlocker control routes and the accounting configuration/payout routes require the existing platform-admin session, with CSRF on mutations and immutable audit events for accounting configuration changes |
