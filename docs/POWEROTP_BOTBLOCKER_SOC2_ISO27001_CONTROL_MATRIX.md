...
| CC6.6 | Boundary protection against external threats | Partially implemented | Phase 11's raw Node wrapper defaults to the socket IP, requires an explicit header/position/trusted-proxy IP list, rejects wildcard trust, bounds paths/headers/bodies, and protects same-origin bridges with a non-simple marker plus Fetch Metadata/Origin checks; other wrappers and deployed edge controls remain later work |
...
| PI1 | Processing integrity — accurate, complete, authorized processing | Partially implemented | Phases 6–11 add atomic sequence controls, request idempotency/replay rejection, scoped credential rotation, transactionally monotonic immutable policy publication, monotonic browser reports, stale/pre-OTP response rejection, trusted wrapper sequence restoration, and loss-safe active-OTP acknowledgement; actual ingestion and decision processing remain Phase 15/20 |
...
| A.5.15 | Access control | Partially implemented | Phase 8 adds independent hashed site credentials, exact host/site/audience binding, customer ownership, platform-admin control routes, CSRF, rate limits, idempotency, and replay controls; Phase 11 keeps the credential server-only behind bounded same-origin raw Node bridges, while production runtime-token authority remains later work |
...
| A.8.26 | Application security requirements | Partially implemented | Phases 9–11 enforce browser authority and sensor cadence/sanitization plus a raw Node wrapper with immediate optimistic delivery, signed clearance, exclusions, limits, explicit proxy trust, same-origin bridges, active-OTP persistence, and authoritative polling; Express/Next.js wrappers and central decision delivery remain later phases |
...
| A.8.29 | Security testing in development and acceptance | Partially implemented | Phase 1–11 suites additionally exercise deterministic browser transitions/sensor timers, late OTP, stale/session/audience/expiry/replay rejection, fail-open versus active challenge persistence, sanitized evidence, raw Node proxy/limit/exclusion/CSRF/cookie boundaries, browser bridge sequencing, authoritative polling/acknowledgement, postMessage non-authority, DOM freeze/focus cleanup, and safe-return rejection; deployed end-to-end acceptance and penetration testing remain Phase 31 |
