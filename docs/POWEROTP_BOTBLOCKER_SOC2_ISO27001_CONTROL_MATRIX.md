...
| A.5.34 | Privacy and protection of PII | Partially implemented | Phase 15 currently activates keyed fingerprint derivation and bounded heatmap/page analytics without page content, email/password duplication, or pointer trails. Phase 17 is approved to replace the changing behavior-report hash with a separate broad browser/device vector and exact HMAC of its stable subset, use authoritative binding then exact-hash matching, prohibit IP-only profile merging, retain session inputs 90 days and aggregate profiles 18 months, and maintain 1/7/30-day system-wide and same-site IP reuse counts as risk evidence. Phase 16 retains raw trusted IP and adds IP-blacklist/ASN/IP-reputation security tables. Phase 21 must place authoritative account/identity PII in Supabase Enterprise and link MongoDB intelligence only through audited opaque/keyed `identityBindings`. DPIA/LIA and privacy-notice review of broad fingerprinting and internal cross-site correlation remain launch prerequisites |
