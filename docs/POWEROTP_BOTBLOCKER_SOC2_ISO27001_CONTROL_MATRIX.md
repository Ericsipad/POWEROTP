...
| CC6.1 | Logical access — least privilege | Partially implemented | Phase 5's configuration GET/PATCH requires an existing customer session and verifies project ownership; Phase 7's public policy read resolves the authoritative customer/project/site scope and returns no credential or private key; runtime site credentials and admin release routes remain later-phase controls |
...
| PI1 | Processing integrity — accurate, complete, authorized processing | Partially implemented | Phase 6 adds atomic sequence/idempotency controls; Phase 7 adds transactionally monotonic immutable policy publication, strict activation/expiry selection, signature re-verification, and rollback refusal; actual ingestion and decision processing remain Phase 15/20 |
...
| A.5.9 | Inventory of information and assets | Partially implemented | Phases 5–7 define `botblockerSites`, `gateSessions`, `userIntelligence`, `riskEvents`, `botblockerChallenges`, and immutable `policyReleases` plus their indexes; entitlement and identity stores remain planned |
...
| A.8.29 | Security testing in development and acceptance | Partially implemented | Phase 1–7 suites exercise contract/retention boundaries, signature/key/replay controls, disabled defaults, timeout boundaries, immutable policy versions, tampering/site/audience/expiry/protocol rejection, ETag/unavailable responses, sequence/idempotency indexes, and cross-project persistence isolation; deployed end-to-end acceptance and penetration testing remain Phase 31 |
