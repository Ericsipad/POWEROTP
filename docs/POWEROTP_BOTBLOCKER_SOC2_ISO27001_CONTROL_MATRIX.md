...
| CC6.6 | Boundary protection against external threats | Partially implemented | Phases 11–13D raw Node/Express/Next wrappers require explicit direct-peer/header/position/trusted-proxy IP settings, optionally enforce exact proxy count, reject wildcard/untrusted/mismatched chains, bound paths/headers/owned bridge bodies, and protect same-origin bridges with a non-simple marker plus Fetch Metadata/Origin checks; Phase 13C validates bounded initial evidence before first contact and keeps both credential and scoped token out of browser state, while Phase 13D replaces spoofable inbound Next request-state and authenticates the bounded internal advisory value; Next omits client IP by default because `NextRequest` has no socket address, and deployed edge controls remain later work. Phase 8A requires a project-scoped `webhookId` URL segment on every site-credential-authenticated runtime route, rejected with a bare 404 before any body parsing or credential/idempotency work if it does not resolve to a real site — removing the ability to reach real logic on a fixed, unscoped global URL, which matters specifically because the platform's fail-open design otherwise lets flooding a shared route degrade every customer at once |
...
| A.5.15 | Access control | Partially implemented | Phase 8 adds independent hashed site credentials, exact host/site/audience binding, customer ownership, platform-admin control routes, CSRF, rate limits, idempotency, and replay controls; Phases 11–13D keep the credential server-only behind bounded same-origin bridges and credential-free browser helpers. Phase 13C uses it only for first contact, stores the returned scoped token only in the Node session, and passes only that token to later adapter service calls; Phase 13D verifies Next client chunks and framework/browser state contain neither value, while production token minting/validation authority remains later work. Phase 15 derives ingestion scope exclusively from the authenticated site, includes customer/project/site scope in every session/event/intelligence read and write, and rejects cross-project session-ID reuse. Phase 21 requires only authoritative account/Passport/entitlement verification to create an internal identity binding; telemetry/customer claims cannot. Phase 14's public anonymous MCP accepts no credential and reads no tenant data |
...
| A.5.34 | Privacy and protection of PII | Partially implemented | Phase 6 enforces scoped persistence, non-unique keyed IP observations, 18-month TTLs, and a 30-day lookup window. Phase 15 activates keyed derivation and bounded heatmap/page analytics without raw IP/fingerprint fields, pointer trails, page content, email/password duplication, or IP-only identity matching. Phase 21 must place authoritative account/identity PII in Supabase Enterprise and link MongoDB intelligence only through audited opaque/keyed `identityBindings`. DPIA/LIA and privacy-notice review remain launch prerequisites |
