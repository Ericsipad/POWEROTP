...
| A.5.34 | Privacy and protection of PII | Partially implemented | Phase 6 enforces scoped persistence, non-unique IP observations, 18-month TTLs, and a 30-day lookup window. Phase 15 activates keyed fingerprint derivation and bounded heatmap/page analytics without browser-supplied fingerprint fields, pointer trails, page content, email/password duplication, or IP-only identity matching. Phase 16's now-shipped network-intelligence work retains the trusted request IP raw rather than hashed, and adds IP-blacklist/ASN-classification/ASN-type-score/IP-reputation-cache tables used only for security decisioning (an exact IP-blacklist match, or an ASN's classification/type score and optional vendor reputation score) — none of it is treated as identity/PII because it is never linked to a Supabase account record, and the raw IP value is separately needed for the site owner's own visitor report and return-visit correlation. Phase 21 must place authoritative account/identity PII in Supabase Enterprise and link MongoDB intelligence only through audited opaque/keyed `identityBindings`. DPIA/LIA and privacy-notice review remain launch prerequisites |
