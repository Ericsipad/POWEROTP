...
| CC6.6 | Boundary protection against external threats | Partially implemented | Phases 11–12 raw Node/Express wrappers default to the socket IP, require explicit header/position/trusted-proxy IP settings, optionally enforce exact proxy count, reject wildcard/untrusted/mismatched chains, bound paths/headers/owned bridge bodies, and protect same-origin bridges with a non-simple marker plus Fetch Metadata/Origin checks; Next.js and deployed edge controls remain later work |
...
| PI1 | Processing integrity — accurate, complete, authorized processing | Partially implemented | Phases 6–12 add atomic sequence controls, request idempotency/replay rejection, scoped credential rotation, transactionally monotonic immutable policy publication, monotonic browser reports, stale/pre-OTP response rejection, trusted wrapper sequence restoration, active-OTP-over-clearance precedence, and loss-safe acknowledgement across raw Node and Express; actual ingestion and decision processing remain Phase 15/20 |
...
| A.5.15 | Access control | Partially implemented | Phase 8 adds independent hashed site credentials, exact host/site/audience binding, customer ownership, platform-admin control routes, CSRF, rate limits, idempotency, and replay controls; Phases 11–12 keep the credential server-only behind bounded same-origin raw Node/Express bridges and credential-free browser helpers, while production runtime-token authority remains later work |
...
| A.8.26 | Application security requirements | Partially implemented | Phases 9–12 enforce browser authority and sensor cadence/sanitization plus raw Node/Express wrappers with immediate optimistic delivery, signed clearance, exclusions, limits, explicit proxy trust/count, same-origin bridges, upload/stream non-interference, active-OTP persistence, and authoritative polling; Next.js and central decision delivery remain later phases |
...
| A.8.29 | Security testing in development and acceptance | Partially implemented | Phase 1–12 suites additionally exercise deterministic browser transitions/sensor timers, late OTP, stale/session/audience/expiry/replay rejection, fail-open versus active challenge persistence, sanitized evidence, raw Node/Express proxy-count/limit/exclusion/CSRF/cookie boundaries, middleware ordering, uploads, streams, compressed responses, pre/post-header errors, WebSocket non-interference, browser bridge sequencing, authoritative polling/acknowledgement, postMessage non-authority, DOM freeze/focus cleanup, and safe-return rejection; deployed end-to-end acceptance and penetration testing remain Phase 31 |
