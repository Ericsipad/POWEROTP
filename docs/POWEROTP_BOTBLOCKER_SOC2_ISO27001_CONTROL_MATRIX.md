...
| CC6.6 | Boundary protection against external threats | Partially implemented | Phases 11–13 raw Node/Express/Next wrappers require explicit direct-peer/header/position/trusted-proxy IP settings, optionally enforce exact proxy count, reject wildcard/untrusted/mismatched chains, bound paths/headers/owned bridge bodies, and protect same-origin bridges with a non-simple marker plus Fetch Metadata/Origin checks; Next omits client IP by default because `NextRequest` has no socket address, and deployed edge controls remain later work |
...
| PI1 | Processing integrity — accurate, complete, authorized processing | Partially implemented | Phases 6–13 add atomic sequence controls, request idempotency/replay rejection, scoped credential rotation, transactionally monotonic immutable policy publication, monotonic browser reports, stale/pre-OTP response rejection, trusted wrapper sequence restoration, active-OTP-over-clearance precedence, and loss-safe acknowledgement across raw Node, Express, and Next.js; actual ingestion and decision processing remain Phase 15/20 |
...
| A.5.15 | Access control | Partially implemented | Phase 8 adds independent hashed site credentials, exact host/site/audience binding, customer ownership, platform-admin control routes, CSRF, rate limits, idempotency, and replay controls; Phases 11–13 keep the credential server-only behind bounded same-origin raw Node/Express/Next bridges and credential-free browser helpers, with Next production chunks checked for credential absence, while production runtime-token authority remains later work |
...
| A.8.26 | Application security requirements | Partially implemented | Phases 9–13 enforce browser authority and sensor cadence/sanitization plus raw Node/Express/Next wrappers with immediate optimistic delivery, signed clearance, exclusions, limits, explicit proxy trust/count, same-origin bridges, upload/stream non-interference, active-OTP persistence, authoritative polling, App Router navigation, and CSP-compatible hosted iframe behavior; central decision delivery remains later work |
...
| A.8.29 | Security testing in development and acceptance | Partially implemented | Phase 1–13 suites additionally exercise deterministic browser transitions/sensor timers, late OTP, stale/session/audience/expiry/replay rejection, fail-open versus active challenge persistence, sanitized evidence, raw Node/Express/Next proxy-count/limit/exclusion/CSRF/cookie boundaries, middleware/App Router ordering, uploads, streams, compressed responses, pre/post-header errors, WebSocket non-interference, browser bridge sequencing, authoritative polling/acknowledgement, postMessage non-authority, DOM freeze/focus cleanup, CSP origin validation, and Next client-bundle credential absence; deployed end-to-end acceptance and penetration testing remain Phase 31 |
