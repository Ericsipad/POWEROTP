...
| PI1 | Processing integrity — accurate, complete, authorized processing | Partially implemented | Phases 6–10 add atomic sequence controls, request idempotency/replay rejection, scoped credential rotation, transactionally monotonic immutable policy publication, monotonic browser reports, and stale/pre-OTP response rejection; actual ingestion and decision processing remain Phase 15/20 |
...
| A.8.26 | Application security requirements | Partially implemented | Phases 9–10 enforce the browser gate authority rules plus five-second/30-second/partial sensor cadence, source sanitization, monotonic ordering, stale-response handling, OTP pause, and authoritative fresh resume; wrappers and central decision delivery remain later phases |
...
| A.8.29 | Security testing in development and acceptance | Partially implemented | Phase 1–10 suites additionally exercise deterministic browser transitions/sensor timers, late OTP, stale/session/audience/expiry/replay rejection, fail-open versus active challenge persistence, sanitized evidence/automation enums, prohibited-data non-emission, authoritative polling, postMessage non-authority, DOM freeze/focus cleanup, and safe-return rejection; deployed end-to-end acceptance and penetration testing remain Phase 31 |
