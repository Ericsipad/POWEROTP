...
import type { BillingTier, CallRateCard, EmailRate, PlanCharge, SmsRateCard } from "@/lib/contracts";
...
export function BillingRatesPanel({ csrfToken }: BillingRatesPanelProps) {
  ...
  async function refresh() {
    const [callResponse, smsResponse, emailResponse, planResponse] = await Promise.all([
      apiFetch("/v1/admin/billing/call-rates", { cache: "no-store" }),
      apiFetch("/v1/admin/billing/sms-rates", { cache: "no-store" }),
      apiFetch("/v1/admin/billing/email-rate", { cache: "no-store" }),
      apiFetch("/v1/admin/billing/plan-charges", { cache: "no-store" }),
    ]);
  ...
  async function saveCallRate(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await apiFetch("/v1/admin/billing/call-rates", {
      method: "PUT",
  ...
  async function saveSmsRate(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const response = await apiFetch("/v1/admin/billing/sms-rates", {
      method: "PUT",
      headers: { ...jsonHeaders, "x-csrf-token": csrfToken },
      body: JSON.stringify({
  ...
  async function saveEmailRate(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const response = await apiFetch("/v1/admin/billing/email-rate", {
      method: "PUT",
      headers: { ...jsonHeaders, "x-csrf-token": csrfToken },
      body: JSON.stringify({
  ...
  async function savePlanCharge(tier: BillingTier, monthlyDisplayUsd: string, dailyChargedUsd: string) {
    const response = await apiFetch("/v1/admin/billing/plan-charges", {
      method: "PUT",
      headers: { ...jsonHeaders, "x-csrf-token": csrfToken },
      body: JSON.stringify({
        tier,
