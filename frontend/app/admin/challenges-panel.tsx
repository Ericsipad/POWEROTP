...
import type { ChallengeDefinition, RecordingAsset } from "@/lib/contracts";
 ...
 * `backend/packages/api/src/challenge-service.ts`.
 */
export function ChallengesPanel({ csrfToken }: ChallengesPanelProps) {
  ...
  async function refresh() {
    const [recordingsResponse, challengesResponse] = await Promise.all([
      apiFetch("/v1/admin/recordings", { cache: "no-store" }),
      apiFetch("/v1/admin/challenges", { cache: "no-store" }),
    ]);
  ...
  async function uploadRecording(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await apiFetch("/v1/admin/recordings", {
      method: "POST",
  ...
  async function retireRecording(id: string) {
    await apiFetch(`/v1/admin/recordings/${id}`, {
      method: "DELETE",
      headers: { "x-csrf-token": csrfToken },
    });
    await refresh();
  ...
  async function createChallenge(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await apiFetch("/v1/admin/challenges", {
      method: "POST",
  ...
  async function retireChallenge(id: string) {
    await apiFetch(`/v1/admin/challenges/${id}`, {
      method: "DELETE",
      headers: { "x-csrf-token": csrfToken },
    });
    await refresh();
