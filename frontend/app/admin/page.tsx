...
import { NODE_STALE_THRESHOLD_MS, type Node, type SessionResponse } from "@/lib/contracts";
...
export default function AdminPage() {
  ...
  async function refreshNodes() {
    const response = await apiFetch("/v1/admin/nodes", {
      cache: "no-store",
    });
  ...
  async function ensureDemoProject() {
    ...
    const response = await apiFetch("/v1/admin/demo-project", {
      method: "POST",
