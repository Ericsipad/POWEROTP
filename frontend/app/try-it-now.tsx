...
import type { VerificationStatus, VerificationType } from "@/lib/contracts";
...
export function TryItNow() {
  ...
  async function poll(interactionId: string, generation: number, attempt: number) {
    ...
    const response = await apiFetch(`/v1/demo/verifications/${interactionId}`, {
      cache: "no-store",
    });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await apiFetch("/v1/demo/verifications", {
        method: "POST",
  ...
  async function submitDemoResponse(
    ...
    const response = await apiFetch(`/v1/demo/verifications/${interactionId}/response`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      ...
      {liveInteractionId && (
        <div className="tryItNowModalPreviewWrap">
          <p className="tryItNowBotNote">
            For Bot Blocker, this OTP challenge will only be shown to suspected bots.
          </p>
