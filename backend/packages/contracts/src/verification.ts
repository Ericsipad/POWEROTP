...
export const InteractionTokenClaimsSchema = z.object({
   ...
   * `backend/packages/api/src/modal-session-service.ts` and `docs/AS_BUILT.md`'s
 ...
 * `backend/packages/api/src/callback-worker.ts`, which already writes one of these on
