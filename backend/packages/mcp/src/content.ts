...
export function buildModalSessionExample(language: "curl" | "typescript") {
  if (language === "curl") {
    return `curl -X POST "https://api.powerotp.com/v1/projects/PROJECT_SLUG/modal-sessions" \\
  -H "Authorization: Bearer $POWEROTP_API_KEY" \\
  -H "Content-Type: application/json" \\
...
const client = new PowerOtpClient({
  ...
  projectUrl: "https://api.powerotp.com/v1/projects/PROJECT_SLUG/verifications",
});
