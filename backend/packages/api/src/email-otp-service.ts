 ...
 * module rather than folded into `backend/packages/api/src/email.ts` (which only ever
 ...
 * `backend/packages/api/src/verification-service.ts`), never POWEROTP's own identity —
...
export function createBrevoEmailOtpService(
  ...
  return {
    async sendOtpCode(targetEmail, code, branding, context) {
      ...
      if (!response.ok) {
        // Same non-2xx-only diagnostic convention as backend/packages/api/src/sms.ts —
