 ...
 * `backend/apps/server/lib/session-cookies.ts#sessionUser`) that already have a
...
export class AuthService {
   ...
   * (`backend/apps/server/app/v1/auth/signup/route.ts`) can decide whether to also
   ...
   * monthly usage quota (`backend/packages/api/src/usage-quota-service.ts`) purely by
