...
import { HostedAuthReturnUrlsSchema } from "@powerotp/contracts";
...
export const GET = apiRoute<RouteParams>(async (request, { params }) => {
  ...
  const response = NextResponse.json(
    await projects.getAuthReturnUrls(authenticated.user._id, projectId),
  );
...
export const PUT = apiRoute<RouteParams>(async (request, { params }) => {
  ...
  const { projectId } = await params;
  const response = NextResponse.json(
    await projects.replaceAuthReturnUrls(
      authenticated.user._id,
      projectId,
      parseBody(HostedAuthReturnUrlsSchema, await request.json()),
      clientIp(request),
    ),
  );
  response.headers.set("cache-control", "no-store");
