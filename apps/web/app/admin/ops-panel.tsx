...
import type { QueueCounts } from "@powerotp/contracts";
import { useState } from "react";

/**
 * Read-only BullMQ queue-depth snapshot for the admin "operator health"
 * view — see `apps/api/src/verification-queue.ts#getQueueCounts` and
...
export function OpsPanel() {
  ...
  async function refresh() {
    const response = await fetch("/v1/admin/queues", {
