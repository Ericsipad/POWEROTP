...
import type { CallbackDeliverySummary } from "@powerotp/contracts";
import { useState } from "react";

/**
 * Read-only visibility into recent callback delivery attempts (both
 * delivered and failed), most recent first — see
 * `apps/api/src/callback-worker.ts` (which already records every attempt)
...
export function CallbackDeliveriesPanel() {
  ...
  async function refresh() {
    const response = await fetch("/v1/admin/callback-deliveries", {
